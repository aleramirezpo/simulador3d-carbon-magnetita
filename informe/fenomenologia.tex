...
\noindent
\ojo{El resultado central} es que el gas del interior se estabiliza en
CO/(CO+CO$_2$) $=$ \datGas{}\,\%, un valor que queda
\emph{entre} dos fronteras termodinámicas: por encima de la que exige la
magnetita (\datUmbralMagnetita{}\,\%) y muy por debajo de la
que exigen la wüstita (\datUmbralWustita{}\,\%) y la ilmenita
(\datUmbralIlmenita{}\,\%). Eso decide, de una sola vez, todo
lo demás: la cascada del hierro avanza hasta wüstita y se para ahí, y la
ilmenita ---que necesita el gas más reductor de todos--- no se mueve.
\medskip
...
que ninguna fase predicha tiene contraste experimental. Lo único medido con lo
que se contrasta es la curva de pérdida de masa (ocho puntos) y tres
observaciones cualitativas de la cronología.
\end{abstract}
...
$\to$ 0). La magnetita desaparece por completo
(\datMagnetitaInicial{} \si{\milli\gram} $\to$ 0). Y toda
esa materia acaba en wüstita: \datFeo{}
\si{\milli\gram}. El hierro metálico se queda en
\datFe{} \si{\milli\gram}, que es apenas el
\datPctFeMetal{}\,\% del hierro total.

\begin{figure}[H]\centering
\includegraphics[width=\linewidth]{fig_fen_hierro.pdf}
\caption{La cascada se detiene en wüstita. A la derecha, el hierro metálico en
su propia escala: son trazas.}
\label{fig:hierro}
\end{figure}
...
CO/(CO+CO$_2$) frente a las tres fronteras. El gas se queda cómodamente por
encima de la de la magnetita, y muy lejos de las otras dos. Ése es todo el
argumento.}
\label{fig:gases}
\end{figure}
...
ofrece \datGas{}\,\%. Faltan cuarenta puntos porcentuales, y
no es una diferencia que se cierre esperando más tiempo: es el estado de
equilibrio del sistema. Para reducir la ilmenita habría que subir la
temperatura bastante por encima de \SI{900}{\celsius}, o cambiar la atmósfera.

Hay además una razón estructural que agrava lo anterior y que viene del DRX:
el hierro de la ilmenita está en \emph{lamas exsueltas dentro de los granos de
titanomagnetita}. Aunque la termodinámica lo permitiera, ese hierro está peor
expuesto que el de la magnetita libre.
...
regeneraría el CO, es demasiado lenta a esta temperatura para mover el sistema
en \SI{720}{\second}.
...
\paragraph{Ninguna fase está validada.} Toda la caracterización del material
(DRX-Rietveld) es \emph{previa} al ensayo. La predicción de que el producto es
wüstita con la ilmenita intacta no se ha comprobado con nada. Bastaría un DRX o
un Mössbauer del aglomerado \emph{después} del ensayo para confirmarla o
tumbarla; es, con diferencia, el dato que más falta hace.
